...
------------------------------------*/
Table Table::Rename (vector<string> oldColumnNames, vector<string> newColumnNames) {
	Table renamedTable (name, primaryKey);
	renamedTable.table = table;

	renamedTable.name = renamedTable.name + " - Renamed";

	vector <int> columnKeys;
	for (int i=0; i<oldColumnNames.size(); i++) {
		columnKeys.push_back(FindColumnKey(oldColumnNames[i]));
	}

	vector <Header> renamedHeader = header;

	for (int i=0; i<columnKeys.size(); i++) {
		int col = columnKeys[i];
		renamedHeader[col].name = newColumnNames[i];
	}
